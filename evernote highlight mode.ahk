...
evernote_url := "evernote.com/client/"

F3::
  ; always enable highlight in evernote
  ...
  If RegExMatch(sURL, evernote_url) {
    MsgBox, % "Start highlight mode"
    last := ""
    Loop {

      ; not in evernote.com anymore: break loop
      sURL := GetActiveBrowserURL()
      If not RegExMatch(sURL, evernote_url) {
        Break
      }

      If Selected() and (clipboard != last) {
        ; highlight
